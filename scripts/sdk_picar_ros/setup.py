...
def install():
    ...
    do(msg="update permissions",
        cmd='run_command("sed -ie \'/^#!/a sudo chmod og+rwx /dev/gpio*\' /etc/rc.local")')
    do(msg="setup i2c on startup",
        cmd='run_command("sed -ie \'/^#[[:space:]][*][[:space:]][[:space:]][*]/a @reboot         root    /etc/rc.local\' /etc/crontab")')
    ...
    if ".sdk-picar" not in listdir("/home/ros"):
        do(msg="create .picar-4wd directory",
            cmd='run_command("sudo mkdir /home/ros/.sdk-picar/")') 
    do(msg="copy picar-4wd-config",
        cmd='run_command("sudo cp ./data/config /home/ros/.sdk-picar/")')
    do(msg="change directory owner",
        cmd='run_command("sudo chown -R ros:ros /home/ros/.sdk-picar/")')
